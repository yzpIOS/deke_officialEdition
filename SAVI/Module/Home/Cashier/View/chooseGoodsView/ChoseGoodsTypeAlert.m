...
-(instancetype)initWithFrame:(CGRect)frame andHeight:(float)height{
    ...
    if (self) {
        ...
        [self addSubview:bgView];
            ...
            goodsInfo = [[GoodsInfoView alloc] initWithFrame:CGRectMake(0, 0, kWidth, kSize(270))];
        ...
        sureButton = [JXUIKit buttonWithBackgroundColor:navigationBackgroundColor titleColorForNormal:WhiteColor titleForNormal:@"确定" titleForSelete:nil titleColorForSelete:nil fontSize:18 font:nil];
        ...
        sureButton.sd_layout.leftSpaceToView(bgView, 0).rightSpaceToView(bgView, 0).bottomSpaceToView(bgView, TopHeight).heightIs(49);
    ...
    for (GoodsTypeModel *model in _dataSource) {
        if (model.selectIndex>=0) {
            if (str.length == 0) {
                ...
                str = [NSString stringWithFormat:@"%@,%@",model.typeArray[model.selectIndex],str];
        }
    }
    return str;
}
...
-(void)textFieldDidEndEditing:(UITextField *)textField
{
   // int count =[countView.countTextField.text intValue];
    ...
    if (textField.text.integerValue <= 0) {
        ...
        countView.countTextField.text = textField.text;
    }
//    if (sizeModel) {
//        if (count >[sizeModel.stock intValue]) {
