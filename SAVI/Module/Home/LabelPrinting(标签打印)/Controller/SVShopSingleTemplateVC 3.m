...
@interface SVShopSingleTemplateVC ()<UITextFieldDelegate,CAAnimationDelegate,UIScrollViewDelegate,UICollectionViewDelegate,UICollectionViewDataSource>
...
- (void)viewDidLoad {
    ...
    self.moneyLabel.text = [NSString stringWithFormat:@"￥%@",_sv_p_unitprice];
    ...
    [[SVSaviTool sharedSaviTool] GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        [self.duoguigeArray enumerateObjectsUsingBlock:^(SVduoguigeModel *duoguigeModel, NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            if (duoguigeModel.sv_p_specs.length) {
                ...
                sizeAttModel.sv_p_barcode = duoguigeModel.sv_p_barcode; // 条码
...
- (void)insert_btnClick:(UIButton *)insert_btn{
    ...
    //   // [self hiddenNO];
    //    UIButton *reduce = self.array3[insert_btn.tag];
    //    reduce.hidden = NO;
    //    UITextField *count_text = self.array4[insert_btn.tag];
    //    count_text.textColor = [UIColor blackColor];
    ////    [insert_btn setImage:[UIImage imageNamed:@"icon_insert"] forState:UIControlStateNormal];
    //    [insert_btn setImage:[UIImage imageNamed:@"icon_insert"] forState:UIControlStateNormal];
}
