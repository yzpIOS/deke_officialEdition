...
#pragma mark - 添加右侧按钮
    if (self.controllerNum == 2) {
        //tabeleView
        self.oneTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 50, ScreenW/8*2, ScreenH-50-TopHeight-50 - 50 - BottomHeight)];
        
         self.twoTableView = [[UITableView alloc]initWithFrame:CGRectMake(ScreenW/8*2, 50, ScreenW/8*6, ScreenH-50-TopHeight-50 - 50 - BottomHeight)];
//         self.twoTableView = [[UITableView alloc]initWithFrame:CGRectMake(ScreenW/6*2, 50, ScreenW/6*4, ScreenH-114)];
        ...
        self.oneTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 50, ScreenW/8*2, ScreenH-50-TopHeight-50 - BottomHeight)];
        
        self.twoTableView = [[UITableView alloc]initWithFrame:CGRectMake(ScreenW/8*2, 50, ScreenW/8*6, ScreenH-50-TopHeight-50 - BottomHeight)];
        ...
        self.oneTableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 50, ScreenW/8*2, ScreenH-50-TopHeight- BottomHeight)];
        
        self.twoTableView = [[UITableView alloc]initWithFrame:CGRectMake(ScreenW/8*2, 50, ScreenW/8*6, ScreenH-50-TopHeight- BottomHeight)];
...
- (void)loadDataWithID:(NSString *)catetoryId{
    ...
    [[SVSaviTool sharedSaviTool] GET:urlStr parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (self.controllerNum == 2) {
            [self setUpCategoryViewArr:arr bottomHeight:100];
        }else{
            [self setUpCategoryViewArr:arr bottomHeight:0];
        }
       // self.controllerNum == 2
...
- (void)setUpCategoryViewArr:(NSArray *)arr bottomHeight:(CGFloat)bottomHeight{
    if (kArrayIsEmpty(arr)) {
        ...
        if (self.controllerNum == 3) {
            self.twoTableView.frame = CGRectMake(ScreenW/8*2, CGRectGetMaxY(self.categoryView.frame), ScreenW/8*6, ScreenH-TopHeight-50-50-BottomHeight-self.categoryView.height - bottomHeight);
        }else{
...
- (void)tagBtnClick:(UIButton *)btn{
        ...
        [self getDataPageIndex:1 pageSize:20 category:[[NSString stringWithFormat:@"%@",self.productcategory_id] integerValue] producttype_id:self.productsubcategory_id name:@"" isn:@"" read_morespec:@"true"];
...
-(void)setupRefresh{
    ...
    MJRefreshBackGifFooter *footer = [MJRefreshBackGifFooter footerWithRefreshingBlock:^{
        ...
        //调用请求
       // [self getDataPageIndex:self.page pageSize:20 category:[[NSString stringWithFormat:@"%@",self.productcategory_id] integerValue] name:@"" isn:@"" read_morespec:@"true"];
        [self getDataPageIndex:self.page pageSize:20 category:[[NSString stringWithFormat:@"%@",self.productcategory_id] integerValue] producttype_id:self.productsubcategory_id name:@"" isn:@"" read_morespec:@"true"];
    }];
    // 设置文字
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar{
   ...
   // [self getDataPageIndex:self.page pageSize:20 category:0 name:keyStr isn:@"" read_morespec:@"true"];
    [self getDataPageIndex:1 pageSize:20 category:[[NSString stringWithFormat:@"%@",self.productcategory_id] integerValue] producttype_id:self.productsubcategory_id name:keyStr isn:keyStr read_morespec:@"true"];
    //移除第一响应者
...
//
//        [weakSelf getDataPageIndex:weakSelf.page pageSize:20 category:[[NSString stringWithFormat:@"%@",weakSelf.productcategory_id] integerValue] producttype_id:weakSelf.productsubcategory_id name:name isn:name read_morespec:@"false"];
//    };
//
//    [self.navigationController pushViewController:VC animated:YES];
    ...
    vc.addShopScanBlock = ^(NSString *resultStr) {
       ...
       // [weakSelf getDataPageIndex:weakSelf.page pageSize:20 category:0 name:name isn:name read_morespec:@"false"];
        ...
        [weakSelf getDataPageIndex:weakSelf.page pageSize:20 category:[[NSString stringWithFormat:@"%@",weakSelf.productcategory_id] integerValue] producttype_id:weakSelf.productsubcategory_id name:resultStr isn:resultStr read_morespec:@"false"];
    };
...
- (void)addWaresButtonTwo{
    ...
    VC.addWaresBlock = ^{
       ...
       // [weakSelf getDataPageIndex:1 pageSize:20 category:[[NSString stringWithFormat:@"%@",weakSelf.productcategory_id] integerValue] name:@"" isn:@"" read_morespec:@"true"];
        weakSelf.page = 1;
        [weakSelf getDataPageIndex:weakSelf.page pageSize:20 category:[[NSString stringWithFormat:@"%@",weakSelf.productcategory_id] integerValue] producttype_id:weakSelf.productsubcategory_id name:@"" isn:@"" read_morespec:@"true"];
    };
    [self.navigationController pushViewController:VC animated:YES];
}
...
//            VC.sv_p_barcode = model.sv_p_barcode;// 款号
                    ...
                    if ([model.isSelect isEqualToString:@"1"]) {
                        for (SVduoguigeModel *selectModel in self.selectModelArray) {
                            if (selectModel.product_id == model.product_id) {
                                ...
                                if (self.removeDuoguigeModel) {
                                    self.removeDuoguigeModel(selectModel);
                                }
...
//
            ...
            cell.sureBtnClickBlock = ^(NSInteger selctCount, SVduoguigeModel * _Nonnull model_two) {
                ...
                if (self.selectModelArray.count == 0) {
                    //第一个选中时添加
                    [self.selectModelArray addObject:model_two];
                    ...
                    for (SVduoguigeModel *selctModel in self.selectModelArray) {
                        ...
                        if (selctModel.product_id == model_two.product_id) {
                            [self.selectModelArray removeObject:selctModel];
                           // aa = 0;
                            break;
                    ...
                    //   当为选中1状态时，添加
                    if ([model_two.isSelect isEqualToString:@"1"]) {
                        [self.selectModelArray addObject:model_two];
