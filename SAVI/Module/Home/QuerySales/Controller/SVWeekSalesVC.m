...
@interface SVWeekSalesVC ()<UITableViewDelegate,UITableViewDataSource>
...
- (void)viewDidLoad {
    ...
    [self getThreeSourcesWithPage:1 top:1 day:2];
    
    self.page = 1;
    //提示加载中
    [SVTool IndeterminateButtonAction:self.tableView withSing:@"加载中…"];
    [self getOrderListWithPage:1 top:20 day:2];
    ...
    MJRefreshBackGifFooter *footer = [MJRefreshBackGifFooter footerWithRefreshingBlock:^{
        ...
        self.page ++;
        //调用请求
        [self getOrderListWithPage:self.page top:20 day:2];
    ...
    [[SVSaviTool sharedSaviTool] GET:sURL parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (![dic[@"values"] isEqual:[NSNull null]]) {
            ...
            for (NSDictionary *dict in self.valuesArr) {
                ...
                //添加到模型数组中
                [self.modelArr addObject:model];    //这里有一报错  1.25
