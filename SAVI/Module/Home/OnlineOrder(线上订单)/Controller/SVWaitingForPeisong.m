...
@interface SVWaitingForPeisong ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation SVWaitingForPeisong
...
- (void)setUpDataPageIndex:(NSInteger)pageIndex pageSize:(NSInteger)pageSize beginDate:(NSString *)beginDate endDate:(NSString *)endDate goodsState:(NSInteger)goodsState numberDay:(NSInteger)numberDay{
    ...
    parames[@"distributionStatus"] = @(0); // 0是等待配送中
...
//           NSArray *order_product_json_strArray = [SVOnlineTobeDelivered mj_JSONObject];
//            NSLog(@"order_product_json_strArray = %@",order_product_json_strArray);
...
- (void)detailClick:(UIButton *)btn{
   ...
   // NSArray *array = [self jsonToObject:model.order_product_json_str];
   // NSLog(@"array = %@",array);
