...
@interface SVMonthAnalysisVC ()<UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource>
...
@implementation SVMonthAnalysisVC
...
//    [self setUpFourView];
    ...
    [self setUpFiveViewWithType:5];
    //商品销售排行type      1 今天，2 昨天，3本月 4 日期筛选（ 填写开始日期和结束日期），5本周，6上周
    [self setUpSixViewWithType:5];
    
    //数据请求      //每日帐单      day:0=今天 -1昨天 7本周 3本月 -2=自定义
    //    [self getSmartAnalysetWithID:2 date:@"2017-08-19" date2:@"2017-08-19" type:0];
    [self GrossProfitWith:7];
    [self getSmartAnalysetWithType:7];
   // [self setUpTwoViewWithType:3];
}
...
#pragma mark - 散客消费统计数据
      ...
      //  NSLog(@"self.bulkguest_1 = %@",valuesDic[@"bulkguestRefund"]);
        ...
        self.V2sumlabel.text = [NSString stringWithFormat:@"%.2f",[self.onePayView.moneylabel.text doubleValue]+[self.fivePayView.moneylabel.text doubleValue]];
...
//        self.V1labelC3.text = [NSString stringWithFormat:@"%.f",[self.twoPayView.moneylabel.text floatValue]+[self.sixPayView.moneylabel.text floatValue]];
        ...
        //饼图 数据
        NSArray *items = @[[PNPieChartDataItem dataItemWithValue:[self.onePayView.moneylabel.text doubleValue] color:colorA],
                           [PNPieChartDataItem dataItemWithValue:[self.fivePayView.moneylabel.text doubleValue] color:colorC],
                           ];
        // 初始化
