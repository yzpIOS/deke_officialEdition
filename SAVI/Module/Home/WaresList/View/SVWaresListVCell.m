...
@interface SVWaresListVCell ()<JJPhotoDelegate>
...
@implementation SVWaresListVCell
...
- (void)updateCell {
    ...
    NSString * sv_p_images = self.model.sv_p_images;
    if (![SVTool isBlankString:self.model.sv_p_images]) {
        ...
        [self.iconImg sd_setImageWithURL:[NSURL URLWithString:[URLHeadPortrait stringByAppendingString:sv_p_images]] placeholderImage:[UIImage imageNamed:@"foodimg"]];
...
- (IBAction)selectButton:(UIButton *)sender {
    ...
    if (self.waresListVCelllock) {

        self.waresListVCelllock(self.model,_model.indexPath);
