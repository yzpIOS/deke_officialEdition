...
-(void)show:(UIView*)contentView Poprect:(CGRect)rect;
{
    ...
    if (_titles.count) {
        for (int index = 0; index < _titles.count; index++) {
            ...
            [title_button setTitleColor:(self.selectIndex == index)?self.selectedColorT:self.noSlectedColor forState:UIControlStateNormal];
            title_button.layer.borderColor = (self.selectIndex == index)?self.selectedColorT.CGColor:self.noSlectedColor.CGColor;
...
-(void)select_button:(UIButton*)sender
{
    if (self.selectIndex != sender.tag) {
        ...
        [sender setTitleColor:self.selectedColorT  forState:UIControlStateNormal];
        sender.layer.borderColor = self.selectedColorT.CGColor;
