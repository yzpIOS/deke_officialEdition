...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self addSubview:btn];
        btn.frame = CGRectMake(0 , ScreenH - 50 ,ScreenW, 50);
     //   [view addSubview:btn];
...
//            }];
        ...
        [self.mainImageView sd_setImageWithURL:[NSURL URLWithString:urlStr] placeholderImage:self.mainImageView.image completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
            if (error == nil) { //下载成功
                [self addGestureRecognizer:self.twoTap];
                self.mainImageView.image = image;
...
//    [self presentViewController:alertControl animated:YES completion:nil];
    ...
    SVAlertView *xlAlertView = [[SVAlertView alloc] initWithTitle:@"保存图片" message:@"" sureBtn:@"确认" cancleBtn:@"取消"];
    if (self.selectNumber == 1) { // 是会员的单图片
        xlAlertView.resultIndex = ^(NSInteger index){
            
        //回调---处理一系列动作
            UIImageWriteToSavedPhotosAlbum(self.mainImageView.image,self,@selector(imageSavedToPhotosAlbum:didFinishSavingWithError:contextInfo:),nil);
        };
    }else{// 是商品的多图片
        xlAlertView.resultIndex = ^(NSInteger index){
            
            [SVUserManager loadUserInfo];
           NSArray *urlArray= [SVUserManager shareInstance].picUrlArr;
            NSString *url = urlArray[self.myindex];
...

{
    ...
    if(!error) {
        ...
        [SVTool TextButtonActionWithSing:message];
