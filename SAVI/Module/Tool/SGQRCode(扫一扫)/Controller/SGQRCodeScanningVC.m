...
- (void)setupNavigationBar {
    ...
    if (device) {
        ...
        if (status == AVAuthorizationStatusNotDetermined) {
            ...
            // 用户拒绝当前应用访问相机
            UIAlertController *alertC = [UIAlertController alertControllerWithTitle:@"⚠️ 警告" message:@"您已关闭访问权限,请去-> [设置 - 隐私 - 相机 - 打开访问开关] 即可使用扫一扫功能" preferredStyle:(UIAlertControllerStyleAlert)];
            UIAlertAction *alertA = [UIAlertAction actionWithTitle:@"设置" style:(UIAlertActionStyleDefault) handler:^(UIAlertAction * _Nonnull action) {
                
                //NSURL *url = [NSURL URLWithString:@"prefs:root=com.xxxx.xxxx"];    // 这里填你App的Bundle ID
                //以上方法在iOS9及之前都work fine，but在iOS10的时候没法跳转，这里将url修改为如下即可，而如下方法在iOS8及以上就支持了。。。
...
//        _scanningView = [SGQRCodeScanningView scanningViewWithFrame:self.view.bounds layer:self.view.layer];
        _scanningView = [SGQRCodeScanningView scanningViewWithFrame:self.view.bounds layer:self.view.layer selectNum:self.selectNum];
    }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects.count > 0) {
        ...
        if (self.selectNum == 1) {
//            [self getDataPageIndex:1 pageSize:1 category:0 name:metaDataObject.stringValue isn:metaDataObject.stringValue read_morespec:@"false"];
            
            if ([self.delegate respondsToSelector:@selector(OneViewControllerCellClick:)]) {
                [self.delegate OneViewControllerCellClick:metaDataObject.stringValue];
            }
