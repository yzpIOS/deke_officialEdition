...
- (instancetype)initWithFrame:(CGRect)frame layer:(CALayer *)layer selectNum:(NSInteger)selectNum{
    if (self = [super initWithFrame:frame]) {
        self.tempLayer = layer;
        self.selectNum = selectNum;
        // 布局扫描界面
        [self setupSubviewsSelectNum:selectNum];
...
+ (instancetype) scanningViewWithFrame:(CGRect)frame layer:(CALayer *)layer selectNum:(NSInteger)selectNum
{
    return [[self alloc] initWithFrame:frame layer:layer selectNum:selectNum];
}

- (void)setupSubviewsSelectNum:(NSInteger)selectNum {
    // 扫描内容的创建
//    CGFloat scanContent_layerX;
//    CGFloat scanContent_layerY;
//    CGFloat scanContent_layerW;
//    CGFloat scanContent_layerH;
    if (selectNum == 1) {
        ...
        CGFloat scanContent_layerH = self.frame.size.width - 3 * scanContent_X;
...
#pragma mark - - - 扫描外部View的创建
        ...
        if (selectNum == 1) {
            ...
            CGFloat textFX = scanContent_X;
            CGFloat textFY = self.frame.size.height * 0.10;
            CGFloat textFW = self.frame.size.width - 2 * scanContent_X;
            CGFloat textFH = 40;
            textF.frame = CGRectMake(textFX, textFY, textFW, textFH);
            // textF.textAlignment = NSTextAlignmentCenter;
            textF.layer.borderColor = navigationBackgroundColor.CGColor;
            textF.layer.borderWidth = 1;
            textF.layer.cornerRadius = 20;
            textF.layer.masksToBounds = YES;
            textF.font = [UIFont systemFontOfSize:14];
            // 就下面这两行是重点    NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:@"请输入占位文字" attributes:

            [SVUserManager loadUserInfo];
            if ([[SVUserManager shareInstance].sv_uit_cache_name isEqualToString:@"cache_name_clothing_and_shoes"]){
                NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:@"请输入商品款号"attributes:
                ...
                NSAttributedString *attrString = [[NSAttributedString alloc] initWithString:@"请输入商品条码"attributes:
                                                  @{NSForegroundColorAttributeName:[UIColor whiteColor],
                                                    NSFontAttributeName:textF.font}];
        ...
        [SVUserManager loadUserInfo];
        if ([[SVUserManager shareInstance].sv_uit_cache_name isEqualToString:@"cache_name_clothing_and_shoes"]){
            promptLabel.text = @"将二维码/款号放入框内, 即可自动扫描";
        }else{
...
- (void)timeAction {
    ...
    if (flag) {
        ...
        if (_scanningline.frame.origin.y >= scanContent_Y) {
            if (self.selectNum == 1) {
                 CGFloat scanContent_MaxY = scanContent_Y + self.frame.size.width - 3 * scanContent_X;
