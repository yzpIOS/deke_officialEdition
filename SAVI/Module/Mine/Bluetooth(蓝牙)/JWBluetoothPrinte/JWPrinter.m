...
- (void)cutter
{
    Byte byteData[] = {0x1d, 0x56, 0x01};
    [_printerData appendBytes:byteData length:sizeof(byteData)];
}
 ...
 *  @param text 文字
 */
- (void)eightSetOffsetText:(NSString *)text
{
    // 1.计算偏移量,因字体和字号不同，所以计算出来的宽度与实际宽度有误差(小字体与22字体计算值接近)
    NSDictionary *dict = @{NSFontAttributeName:[UIFont systemFontOfSize:22.0]};
    NSAttributedString *valueAttr = [[NSAttributedString alloc] initWithString:text attributes:dict];
...
- (void)appendTitle:(NSString *)title value:(NSString *)value fontSize:(HLFontSize)fontSize
{
    ...
    [self setText:title];
    // 4.设置实际值
    //    [self setOffsetText:value];//这个有偏移量
    [self setText:value];
    // 5.换行
    [self appendNewLine];
    if (fontSize != HLFontSizeTitleSmalle) {
...
- (void)appendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle
{
    ...
    if (left) {
       ...
       // [self setText:left maxChar:10];
        ...
        [self setText:left maxChar:15];

    }
    
    if (middle) {
        [self setOffset:180 + offset];
        [self setText:middle];
    }
    
    if (right) {

      //  [self setOffset:270 + offset];

        

        [self setOffset:280 + offset];
...
#pragma mark - 80小票格式
- (void)eightAppendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle
{
    [self setAlignment:HLTextAlignmentLeft];
    [self setFontSize:HLFontSizeTitleSmalle];
    NSInteger offset = 0;
    ...
    if (middle) {
        [self setOffset:260 + offset];
        [self setText:middle];
    }
    
    if (right) {
        [self setOffset:460 + offset];
        [self setText:right];
    }
...
// 自己写的58格式
- (void)twoAppendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle{
    [self setAlignment:HLTextAlignmentLeft];
    [self setFontSize:HLFontSizeTitleSmalle];
    NSInteger offset = 0;
    if (!isTitle) {
    ...
    if (left) {
        [self setText:left maxChar:35];
    }
    
    if (middle) {
        [self setOffset:340 + offset];
        [self setText:middle];
    }
...
// 自己写的80格式
- (void)eightTwoAppendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle{
    [self setAlignment:HLTextAlignmentLeft];
    [self setFontSize:HLFontSizeTitleSmalle];
    NSInteger offset = 0;
    if (!isTitle) {
    ...
    if (middle) {
        [self setOffset:520 + offset];
        [self setText:middle];
    }
    
    if (right) {
        [self eightSetOffsetText:right];
    }
...
// 自己写的
- (void)threeAppendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle{
    [self setAlignment:HLTextAlignmentLeft];
    [self setFontSize:HLFontSizeTitleSmalle];
    NSInteger offset = 0;
    if (!isTitle) {
...
// 自己写的
- (void)eightThreeAppendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right isTitle:(BOOL)isTitle{
    [self setAlignment:HLTextAlignmentLeft];
    [self setFontSize:HLFontSizeTitleSmalle];
    NSInteger offset = 0;
    if (!isTitle) {
...
//自己写的
- (void)appendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right priceText:(NSString *)price isTitle:(BOOL)isTitle
{
    [self setAlignment:HLTextAlignmentLeft];
    [self setFontSize:HLFontSizeTitleSmalle];
    NSInteger offset = 0;
    ...
    if (left) {
        [self setText:left maxChar:13];
    }
    
    if (middle) {
        if (!isTitle) {
            [self setOffset:130 + offset];
        } else {
            // 1.计算偏移量,因字体和字号不同，所以计算出来的宽度与实际宽度有误差(小字体与22字体计算值接近)
            NSDictionary *dict = @{NSFontAttributeName:[UIFont systemFontOfSize:22.0]};
    ...
    if (price) {
        ...
        //        } else {
        //            [self setOffset:330 + offset];
        //        }
        [self setOffsetText:price];
    }
...
/**
 自己写的80
 */
- (void)eightAppendLeftText:(NSString *)left middleText:(NSString *)middle rightText:(NSString *)right priceText:(NSString *)price isTitle:(BOOL)isTitle
{
    [self setAlignment:HLTextAlignmentLeft];
    [self setFontSize:HLFontSizeTitleSmalle];
    NSInteger offset = 0;
...
{
    [self appendBarCodeWithInfo:info alignment:HLTextAlignmentLeft maxWidth:300];
}
